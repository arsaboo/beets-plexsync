...
class PlexSync(BeetsPlugin):
    ...
    def commands(self):
        ...
        def func(lib, opts, args):
            from beetsplug.agent import BeetsAgent
            agent = BeetsAgent(lib, self.cache)
            agent.run("update_library")
        ...
        def func_sync(lib, opts, args):
            ...
            agent.run("sync", items=items, write=ui.should_write(), force=opts.force_refetch)
        ...
        def func_playlist_add(lib, opts, args):
            ...
            agent = BeetsAgent(lib, self.cache)
            items = lib.items(ui.decargs(args))
            agent.run("add_to_playlist", items=items, playlist_name=opts.playlist)
        ...
        def func_playlist_rem(lib, opts, args):
            ...
            agent = BeetsAgent(lib, self.cache)
            items = lib.items(ui.decargs(args))
            agent.run("remove_from_playlist", items=items, playlist_name=opts.playlist)
        ...
        def func_sync_recent(lib, opts, args):
            from beetsplug.agent import BeetsAgent
            agent = BeetsAgent(lib, self.cache)
            agent.run("sync_recent", days=opts.days)
        ...
        def func_playlist_import(lib, opts, args):
            from beetsplug.agent import BeetsAgent
            agent = BeetsAgent(lib, self.cache)
            agent.run("import_playlist", playlist_name=opts.playlist, url=opts.url)
    ...
    def _plex_import_playlist(self, playlist, playlist_url=None, listenbrainz=False):
        ...
        if listenbrainz:
            ...
            importer = PlaylistImporter(self.cache)
            songs = importer.run(playlist_url)

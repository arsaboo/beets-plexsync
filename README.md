...
* `beet plexplaylistimport`: allows you to import playlists from other online services. Apple Music and Spotify are currently supported (more coming soon). Use the `-m` flag to specify the playlist name to be created in Plex and supply the full playlist url with the `-u` flag.
